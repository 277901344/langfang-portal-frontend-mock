...
const STORAGE_KEY = 'jingji_portal_demands_v4';
...
export function submitDemandResponse(
  ...
  const updated = list.map((d) => {
    if (d.id === demandId) {
      // Also sync into trading center responded list
      try {
        const tradingResps = getStoredRespondDemands();
        const newTradingResp: TradingRespondDemand = {
          id: `RSP${new Date().toISOString().slice(0, 10).replace(/-/g, '')}${Math.floor(100 + Math.random() * 900)}`,
          demandId: d.id,
          demandName: d.title,
          publisherOrg: d.publisherOrg || '需求发起机构',
          demandType: d.productType || '数据集',
          industry: d.topicCategory || '通用领域',
          deliveryFormat: response.deliveryType || 'API接口',
          myPricingType: response.pricingType || '按项目交付',
          myQuoteAmount: response.quoteAmount ? `¥${response.quoteAmount}` : '商务面议',
          myDeliveryType: response.deliveryType || 'API接口',
          myRelatedProduct: response.relatedProduct || '平台数据资产',
          mySolutionDesc: response.solutionDesc,
          responseResult: '待确认',
          deadline: d.deadline || '2026-10-31',
          respondTime: dateFormatted,
          responder: response.responder || 'wangui',
        };
        saveRespondDemands([newTradingResp, ...tradingResps.filter((x) => x.demandId !== d.id)]);
      } catch (err) {
        console.error('Failed to sync response to trading demands store', err);
      }

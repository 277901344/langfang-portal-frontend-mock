...
export type DemandStatus = '未发布' | '已发布' | '已有响应' | '已匹配' | '已关闭' | '已过期';
...
export interface DemandItem {
  ...
  productType?: string; // 产品类型: 数据集 / API产品 / 数据应用 / 数据报告 / 数字对象 / 其他
  deliveryFormat?: string; // 兼容字段
  updateFrequency?: string; // 更新频次
  usePurpose?: string; // 使用目的
  budgetType?: string; // 预算类型: 免费 / 按次计费 / 包月
...
export const DEMAND_STATUSES = ['不限', '已发布', '已有响应', '已匹配', '已关闭', '已过期', '未发布'];
export const DEMAND_PRODUCT_TYPES = ['不限', '数据集', 'API产品', '数据应用', '数据报告', '数字对象', '其他'];
export const DEMAND_BUDGET_TYPES = ['不限', '免费', '按次计费', '包月'];

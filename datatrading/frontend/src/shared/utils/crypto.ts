...
const DEFAULT_CUSTOM_KEY = 'ZfVrWWDJRfspwnQ1jSWKEA==';
...
function aesDecrypt(encryptedHex: string, key: string): string {
...
function convertToWordArray(text: string): number[] {
    ...
    while (byteCount < length) {
        ...
        wordArray[wordIndex] = wordArray[wordIndex] | (utf8.charCodeAt(byteCount) << bytePosition);
        byteCount++;
    }
...
function wordToHex(value: number): string {
    ...
    for (let i = 0; i <= 3; i++) {
        ...
        output += hex.substring(hex.length - 2);
    }
    return output;
}
...
export { aesEncrypt, aesDecrypt, DEFAULT_CUSTOM_KEY, md5 };

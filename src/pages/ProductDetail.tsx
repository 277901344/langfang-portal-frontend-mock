...
import {
  ...
  CreditCard, Calendar, BadgePercent,
  type LucideIcon,
} from 'lucide-react';
...
export default function ProductDetail() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24 selection:bg-blue-500/20">
      ...
      <div className="relative overflow-hidden bg-gradient-to-br from-slate-50 via-white to-sky-50/30 border-b border-slate-200 pt-10 pb-12">
        ...
        <div className="relative w-full max-w-[1680px] mx-auto px-4 sm:px-6 lg:px-10">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
            ...
            {/* Right Info Card */}
            <div className="w-full lg:w-80 shrink-0 bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <div className="mb-4 p-4 rounded-xl bg-slate-50 border border-slate-200/80">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs text-slate-500 font-medium">定价模式</span>
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-bold ${
                      record.pricingInfo?.type === '免费' 
                        ? 'bg-emerald-50 text-emerald-700 border border-emerald-200'
                        : record.pricingInfo?.type === '包月'
                  ...
                  {/* 价格展示 */}
                  {(!record.pricingInfo || record.pricingInfo.type === '免费') && (
                    <div className="flex items-baseline gap-1 text-emerald-600 font-extrabold my-1">
                      <span className="text-2xl font-black tracking-tight">免费</span>
                      <span className="text-xs text-slate-400 font-normal">/ 开放调用</span>
                    </div>
                  )}

                  {record.pricingInfo?.type === '按次计费' && (
                    <div className="space-y-1 my-1">
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-xs font-bold text-slate-900">¥</span>
                        <span className="text-2xl font-black tracking-tight text-slate-900">
                          {Number(record.pricingInfo.discountUnitPrice ?? record.price ?? 0).toFixed(2)}
                        </span>
                        <span className="text-xs font-semibold text-slate-500">/次</span>
                        {record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                          <span className="ml-auto px-1.5 py-0.5 rounded bg-rose-50 border border-rose-200 text-rose-600 text-[11px] font-bold">
                            {formatDiscountRate(record.pricingInfo.discountPercent)}
                          </span>
                        )}
                      </div>
                      {record.pricingInfo.unitPrice !== undefined && record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                        <div className="text-xs text-slate-400 line-through">
                          原价: ¥{Number(record.pricingInfo.unitPrice).toFixed(2)}/次
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {record.pricingInfo?.type === '包月' && (
                    <div className="space-y-1 my-1">
                      <div className="flex items-baseline gap-1.5">
                        <span className="text-xs font-bold text-slate-900">¥</span>
                        <span className="text-2xl font-black tracking-tight text-slate-900">
                          {Number(record.pricingInfo.discountMonthlyPrice ?? record.price ?? 0).toFixed(2)}
                        </span>
                        <span className="text-xs font-semibold text-slate-500">/月</span>
                        {record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                          <span className="ml-auto px-1.5 py-0.5 rounded bg-rose-50 border border-rose-200 text-rose-600 text-[11px] font-bold">
                            {formatDiscountRate(record.pricingInfo.discountPercent)}
                          </span>
                      ...
                      {record.pricingInfo.monthlyPrice !== undefined && record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                        <div className="text-xs text-slate-400 line-through">
                          原价: ¥{Number(record.pricingInfo.monthlyPrice).toFixed(2)}/月
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  <div className="mt-3 pt-2.5 border-t border-slate-200 flex items-center justify-between text-xs">
                    <span className="text-slate-500 font-medium">服务有效期</span>
                    <span className="font-semibold text-slate-800">
                      {record.validityInfo?.type === '自定义' && record.validityInfo.expireTime
                        ? record.validityInfo.expireTime
                        : '永久'}
                    </span>
                  </div>
                </div>
              ...
              {/* Action Button - 严格保持原样，按钮不要变 */}
              <div className="pt-4 border-t border-slate-200">
                <button
                  type="button"
                  onClick={() => {
                    if (
                      record.productId === 'LF-DP-004' ||
                      record.productId === 'LF-DP-008' ||
                      record.productId === 'dp-004' ||
                      record.productId === 'dp-008' ||
                      record.productName.includes('不动产确权真伪核验') ||
                      record.productName.includes('不动产抵押及查封状态')
                    ) {
      ...
      {/* Main Content */}
      <div className="w-full max-w-[1680px] mx-auto mt-8 space-y-6 px-4 sm:px-6 lg:px-10">
        ...
        {/* 2. Middle Row: 交付与计费 (包含定价模式与价格体系、服务有效期) */}
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-2xs overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">明确产品定价模式、价格体系、有效期及交付规则</p>
              </div>
            </div>
          </div>

          <div className="divide-y divide-slate-100 text-xs">
            {/* Section 1: 定价模式与价格体系明细 */}
            <div className="bg-slate-50/50 p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs">
                  <div className="text-slate-400 font-normal mb-1.5 flex items-center justify-between">
                    <span>定价模式</span>
                    <span className="text-[10px] text-slate-400">收费类别</span>
                  </div>
                  <div className="text-sm font-bold text-slate-900 flex items-center gap-2">
                    <span className={`inline-block w-2 h-2 rounded-full ${
                      record.pricingInfo?.type === '免费' ? 'bg-emerald-500' : 'bg-blue-500'
                    }`} />
                    {record.pricingInfo?.type || '免费'}
                  </div>
                  <div className="text-[11px] text-slate-500 mt-1">
                    {record.pricingInfo?.type === '免费' && '向合规准入机构免费提供数据调用服务'}
                    {record.pricingInfo?.type === '按次计费' && '按照实际调用/核验成功的次数进行阶梯核算'}
                    {record.pricingInfo?.type === '包月' && '按月度订阅周期享受不限次/配额内的数据接入'}
                  </div>
                </div>
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-2xs">
                  ...
                  {(!record.pricingInfo || record.pricingInfo.type === '免费') && (
                    <div>
                      ...
                      <div className="text-[11px] text-slate-500 mt-1">原价 ¥0.00，当前无需付费</div>
                    </div>
                  )}

                  {record.pricingInfo?.type === '按次计费' && (
                    <div>
                      <div className="flex items-baseline gap-1.5 text-slate-900 font-extrabold">
                        ...
                        {record.pricingInfo.unitPrice !== undefined && record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                          <span className="text-xs text-slate-400 line-through ml-1 font-normal">
                            ¥{Number(record.pricingInfo.unitPrice).toFixed(2)}
                          </span>
                        )}
                      </div>
                      <div className="text-[11px] text-slate-500 mt-1">
                        单次价格 ¥{Number(record.pricingInfo.unitPrice ?? 0.10).toFixed(2)}，折扣 {record.pricingInfo.discountPercent ?? 100}%
                      </div>
                    </div>
                  )}
                  ...
                  {record.pricingInfo?.type === '包月' && (
                    <div>
                      <div className="flex items-baseline gap-1.5 text-slate-900 font-extrabold">
                        ...
                        {record.pricingInfo.monthlyPrice !== undefined && record.pricingInfo.discountPercent && record.pricingInfo.discountPercent < 100 && (
                          <span className="text-xs text-slate-400 line-through ml-1 font-normal">
                            ¥{Number(record.pricingInfo.monthlyPrice).toFixed(2)}
                          </span>
                        )}
                      </div>
                      <div className="text-[11px] text-slate-500 mt-1">
                        包月价格 ¥{Number(record.pricingInfo.monthlyPrice ?? 200.00).toFixed(2)}，折扣 {record.pricingInfo.discountPercent ?? 100}%
                      </div>
                    </div>
                  )}
                </div>
            ...
            {/* Section 2: 交付属性明细 */}
            <div className="grid grid-cols-2 md:grid-cols-2 gap-4 px-6 py-3.5">
              <div>

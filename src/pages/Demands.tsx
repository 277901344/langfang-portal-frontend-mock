...
export function Demands() {
  ...
  // Add expected field tag
  const handleAddExpectedField = () => {
    if (fieldInput.trim() && !expectedFieldsList.includes(fieldInput.trim())) {
      setExpectedFieldsList([...expectedFieldsList, fieldInput.trim()]);
      setFieldInput('');
    }
  };
  ...
  const handlePublishSubmit = (e: React.FormEvent) => {
    ...
    const newDemand = addDemand({
      ...
      productType: publishProductType || 'API接口',
      ...
      expectedFields: expectedFieldsList.length > 0 ? expectedFieldsList : ['统一社会信用代码', '企业名称', '法定代表人'],
    ...
    <div className="min-h-screen bg-slate-50 pb-24 font-sans selection:bg-blue-500/20">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-50 via-white to-sky-50/30 border-b border-slate-200 pt-14 pb-16 sm:pt-16 sm:pb-20">
            <div className="relative w-full max-w-[1680px] mx-auto px-4 sm:px-6 lg:px-10">
              <div className="mb-6 flex items-center gap-2 text-sm text-slate-500">
                ...
                <ChevronRight className="h-4 w-4 text-slate-400" />
                <span className="line-clamp-1 text-slate-900 font-medium">发布数据需求</span>
              </div>
              ...
              <div className="max-w-3xl space-y-2">
                ...
                <p className="text-sm text-slate-500">
                  填写并提交您的数据需求，平台将通过智能撮合与跨域可信数据方案连接权威数据提供方。
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-8">
                ...
                <div className="space-y-4 text-xs text-slate-700">
                  <div className="space-y-1.5">
                    ...
                    <input
                      ...
                      className="w-full h-10 border border-slate-200 rounded-xl px-3 bg-white text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-1.5">
                      ...
                      <select
                        ...
                        className="w-full h-10 border border-slate-200 rounded-xl px-3 bg-white text-xs text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                      >
                    ...
                    <div className="space-y-1.5">
                      ...
                      >
                        ...
                        <option value="信贷风控">信贷风控</option>
                        <option value="城市公共交通">城市公共交通</option>
                        <option value="医疗科研">医疗科研</option>
                        <option value="供应链金融">供应链金融</option>
                        <option value="智慧城市">智慧城市</option>
                        <option value="气象预测">气象预测</option>
                      </select>
                    </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full h-10 border border-slate-200 rounded-xl px-3 pr-9 bg-white text-xs text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20"
                      />
              ...
              <div className="space-y-4 border-t border-slate-100 pt-6">
                ...
                <div className="space-y-4 text-xs text-slate-700">
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        className="w-full border border-slate-200 rounded-xl p-3 text-xs text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 bg-white"
                      />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="border border-dashed border-slate-200 rounded-md p-4 bg-slate-50/50 flex flex-col items-center justify-center space-y-2">
                      {expectedFieldsList.length > 0 && (
                        <div className="flex flex-wrap gap-2 w-full mb-2">
                          {expectedFieldsList.map((f, idx) => (
                            <span key={idx} className="inline-flex items-center gap-1 bg-white border border-slate-200 px-2.5 py-1 rounded text-xs text-slate-700">
                              {f}
                              <button
                                type="button"
                                onClick={() => setExpectedFieldsList(expectedFieldsList.filter((_, i) => i !== idx))}
                      ...
                      {showFieldInput ? (
                        <div className="flex items-center gap-2 w-full max-w-md">
                          <input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddExpectedField())}
                            placeholder="输入期望字段名称并按回车"
                            className="flex-1 h-8 border border-slate-200 rounded px-2 text-xs bg-white focus:outline-none"
                          />
                          <button
                            type="button"
                            onClick={handleAddExpectedField}
              ...
              <div className="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
                <button
                  type="button"
                  onClick={() => setViewMode('list')}
                  className="px-6 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 text-xs font-medium hover:bg-slate-50 cursor-pointer transition-colors"
                >
                  取 消
                </button>
                <button
                  type="submit"
                  className="px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold shadow-xs cursor-pointer transition-colors"
                >
                  创建需求
                </button>
              </div>
      ...
      {viewMode === 'detail' && activeDemand && (
        <div className="min-h-screen bg-slate-50 pb-24">
          ...
          <div className="relative overflow-hidden bg-gradient-to-br from-slate-50 via-white to-sky-50/30 border-b border-slate-200 pt-14 pb-16 sm:pt-16 sm:pb-20">
            <div className="relative w-full max-w-[1680px] mx-auto px-4 sm:px-6 lg:px-10">
              <div className="mb-6 flex items-center gap-2 text-sm text-slate-500">
                ...
                <ChevronRight className="h-4 w-4 text-slate-400" />
                <span className="line-clamp-1 text-slate-900 font-medium">{activeDemand.title}</span>
              </div>
              ...
              <div className="flex flex-col lg:flex-row justify-between items-start gap-6">
                <div className="max-w-3xl space-y-4">
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap">
                    <span>发布时间：<span className="font-mono text-slate-700">{activeDemand.createdAt}</span></span>
                    <span>·</span>
                    <span>截止日期：<span className="font-mono text-slate-700">{activeDemand.deadline}</span></span>
                  </div>
                </div>

                {/* Role perspective switcher */}
                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-1.5 bg-white border border-slate-200 rounded-xl p-1.5 shadow-2xs text-xs shrink-0">
                  <span className="text-slate-400 font-medium px-2 text-[11px]">切换视角：</span>
                  <button
                    type="button"
                    onClick={() => setPerspectiveRole('provider')}
                    className={cn(
                      "px-3 py-1.5 rounded-lg font-medium cursor-pointer transition-colors text-xs",
                      perspectiveRole === 'provider'
                        ? "bg-blue-600 text-white font-bold shadow-2xs"
                        : "text-slate-600 hover:bg-slate-100"
                    )}
                  >
                    数据提供方 (Ifsc_admin / 廊坊市***科技)
                  </button>
                  <button
                    type="button"
          ...
          <div className="w-full max-w-[1680px] mx-auto px-4 sm:px-6 lg:px-10 mt-8">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-8 relative">
              <div className="space-y-4 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-y-3.5 gap-x-6 text-xs text-slate-600 bg-slate-50/70 p-4 rounded-xl border border-slate-100">
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">需求编号:</span>
                    <span className="text-slate-800 font-mono font-medium">{activeDemand.id}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">发布时间:</span>
                    <span className="text-slate-800 font-mono font-medium">{activeDemand.createdAt}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">截止日期:</span>
                    <span className="text-slate-800 font-mono font-medium">{activeDemand.deadline}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">主题分类:</span>
                    <span className="text-slate-800 font-medium">{activeDemand.topicCategory}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">应用场景:</span>
                    <span className="text-slate-800 font-medium">{activeDemand.applicationScene || '-'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">交付形式:</span>
                    <span className="text-slate-800 font-medium">{activeDemand.productType || '数据接口 / 数据集'}</span>
                  </div>

                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">更新频次:</span>
                    <span className="text-slate-800 font-medium">{activeDemand.updateFrequency || '每周'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-slate-400 shrink-0">响应数量:</span>
                    <span className="text-blue-600 font-bold">{activeDemand.responses.length} 个响应</span>
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-3 border-t border-slate-100 pt-6">
              ...
              <div className="text-xs text-slate-700 space-y-1.5 pl-3">
                <div className="font-medium text-slate-800">需求描述</div>
            ...
            <div className="space-y-3 border-t border-slate-100 pt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-y-3 gap-x-6 text-xs text-slate-600 pl-3">
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 shrink-0">预算类型:</span>
                  <span className="text-slate-800">{activeDemand.budgetType || '免费'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 shrink-0">预算金额:</span>
                  <span className="text-slate-800">{activeDemand.budgetAmount || '-'}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 shrink-0">使用目的:</span>
              ...
              <div className="space-y-4 border-t border-slate-100 pt-6">
                ...
                <div className="space-y-3 pl-3">
                  {activeDemand.responses.map((resp) => (
                    <div
                      key={resp.id}
                      className="border border-slate-200 bg-slate-50/70 rounded-xl p-4 space-y-2 text-xs text-slate-700"
                    >
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-900">我的响应</span>
                        <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[11px] font-bold px-2 py-0.5 rounded-md">
                          {resp.status}
                        </span>
                      </div>
            ...
            {perspectiveRole === 'publisher' && (
              <div className="space-y-6 border-t border-slate-100 pt-6">
                  ...
                  <div className="space-y-3">
                    {activeDemand.responses.length > 0 ? (
                      activeDemand.responses.map((resp) => (
                        <div
                          key={resp.id}
                          className="border border-slate-200 bg-slate-50/70 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs text-slate-700"
                        >
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-slate-900">{resp.responder}</span>
                              <span className="bg-blue-50 text-blue-700 border border-blue-200 text-[11px] font-bold px-2 py-0.5 rounded-md">
                                {resp.status}
                              </span>
                            </div>
                          ...
                          {resp.status === '待处理' && (
                            <div className="flex items-center gap-2 shrink-0 self-start md:self-center">
                              ...
                              <button
                                ...
                                className="px-4 py-1.5 bg-white border border-rose-300 text-rose-600 hover:bg-rose-50 rounded-lg text-xs font-bold cursor-pointer transition-colors"
                              >
                                拒绝
                              </button>
                            </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                  {activeDemand.status === '未发布' ? (
                    <>
                      <button
                        type="button"
                        onClick={() => {
                          setPublishTitle(activeDemand.title);
                          setPublishTopic(activeDemand.topicCategory === '-' ? '' : activeDemand.topicCategory);
                          setPublishScene(activeDemand.applicationScene === '-' ? '' : (activeDemand.applicationScene || ''));
                          setPublishProductType(activeDemand.productType === '-' ? '' : (activeDemand.productType || ''));
                          setPublishUpdateFreq(activeDemand.updateFrequency || '');
                          setPublishDeadline(activeDemand.deadline || '2026-08-29');
                          setPublishPurpose(activeDemand.usePurpose === '-' ? '' : (activeDemand.usePurpose || ''));
                          setPublishDesc(activeDemand.description === '暂无详细描述' ? '' : activeDemand.description);
                          setPublishBudgetType(activeDemand.budgetType || '');
                          setPublishBudgetAmount(activeDemand.budgetAmount || '');
                          setViewMode('publish');
                        }}
                        className="px-5 py-2 border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 rounded-md text-xs font-medium cursor-pointer transition-colors shadow-2xs"
                      >
                        编辑需求
                      </button>
                      ...
                      <button
                        ...
                        className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold cursor-pointer shadow-xs transition-colors"
                      >
                        发布需求
                      </button>
                      ...
                      <button
                        ...
                        className="px-5 py-2.5 border border-rose-300 text-rose-600 hover:bg-rose-50 rounded-xl text-xs font-bold cursor-pointer transition-colors shadow-2xs"
                      >
                        关闭需求
                      </button>
                    </>
                    ...
                    <button
                      ...
                      className="px-5 py-2.5 border border-rose-300 text-rose-600 hover:bg-rose-50 disabled:opacity-50 rounded-xl text-xs font-bold transition-colors cursor-pointer shadow-2xs"
                    >
                      关闭需求
                    </button>
                  )}
              ...
              <form onSubmit={handleResponseSubmit} className="p-6 space-y-4 text-xs text-slate-700">
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <textarea
                      ...
                      placeholder="请输入对齐方案与能力描述"
                      className="w-full border border-slate-200 rounded-xl p-3 text-xs text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 bg-white"
                    />
                    <div className="absolute right-3 bottom-2 text-[11px] text-slate-400 font-mono">
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                  <button
                    type="button"
                    onClick={() => setIsRespondModalOpen(false)}
                    className="px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-medium hover:bg-slate-50 cursor-pointer transition-colors"
                  >
                    取 消
                  </button>
                  <button
                    type="submit"
                    className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold cursor-pointer transition-colors shadow-xs"
                  >
                    提交响应
                  </button>
                </div>

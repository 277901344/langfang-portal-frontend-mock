...
export function Demands() {
  ...
  const handlePublishSubmit = (e: React.FormEvent) => {
    ...
    const newDemand: DemandItem = {
      id: `DM-${todayStr.replace(/-/g, '')}-000${demands.length + 1}`,
      ...
      topicCategory: publishTopic || '-',
      applicationScene: publishScene || '-',
      productType: publishProductType || '-',
      updateFrequency: publishUpdateFreq || '每周',
      usePurpose: publishPurpose || '-',
      budgetType: publishBudgetType || '免费',
      budgetAmount: publishBudgetAmount || '-',
      datasetRequirement: publishDesc.trim() || '通用测试数据集需求',
      expectedFields: expectedFieldsList,
      deadline: publishDeadline || '2026-08-29',
      responseCount: 0,
      createdAt: `${todayStr} ${timeStr}`,
      budget: publishBudgetType === '免费' ? '免费' : (publishBudgetAmount || '面议'),
  ...
  const handleResponseSubmit = (e: React.FormEvent) => {
    ...
    const dateFormatted = `${todayNow.toISOString().slice(0, 10)} ${todayNow.toTimeString().slice(0, 5)}`;

    const newResp: DemandResponseItem = {
      id: `RESP-${Date.now()}`,
      responder: 'Ifsc_admin',
    ...
    const updatedList = demands.map((d) => {
      if (d.id === activeDemand.id) {
        return {
          ...d,
          status: '已有响应' as const,
          responseCount: d.responses.length + 1,
    ...
    const updatedDemand = {
      ...activeDemand,
      status: '已有响应' as const,
      responseCount: activeDemand.responses.length + 1,
      responses: [...activeDemand.responses, newResp],
    };
                      ...
                      >
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex items-center gap-1.5 text-xs text-slate-500 font-medium">
                            ...
                            <span className="truncate">{item.publisherOrg || item.publisher}</span>
                          </div>
                          ...
                          <div className="mt-3.5 grid grid-cols-2 divide-x divide-slate-100 border-y border-slate-100 py-2.5">
                            <div className="min-w-0 pr-4">
                              <div className="text-[10px] font-semibold tracking-wide text-slate-400">更新频次</div>
                              <div className="mt-0.5 truncate text-xs font-semibold text-slate-700">{item.updateFrequency || '每周'}</div>
                            </div>
                            <div className="min-w-0 pl-4">
                              <div className="text-[10px] font-semibold tracking-wide text-slate-400">截止日期</div>
                              <div className="mt-0.5 truncate text-xs font-semibold text-slate-700 font-mono">
                                {item.deadline || '-'}
                              </div>
                            </div>
                          </div>
      ...
      {viewMode === 'detail' && activeDemand && (
        <div className="max-w-4xl mx-auto px-4 py-8 space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-2 border-b border-slate-200/80">
            ...
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-xl p-1 shadow-2xs text-[11px]">
              <span className="text-slate-400 font-medium px-2">切换视角：</span>
              <button
                type="button"
                onClick={() => setPerspectiveRole('provider')}
                className={cn(
                  "px-3 py-1 rounded-lg font-medium cursor-pointer transition-colors",
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 md:p-8 space-y-8 relative">
            <div className="space-y-4 relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-y-3 gap-x-6 text-xs text-slate-600 pt-2">
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 shrink-0">需求编号:</span>
                  <span className="text-slate-800 font-mono">{activeDemand.id}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-slate-400 shrink-0">发布方:</span>
                  <span className="text-slate-800">{activeDemand.publisher}</span>
                </div>
                <div className="flex items-center gap-2">

          ...
          <div className="mx-auto max-w-[1160px] overflow-hidden rounded-2xl border border-[#C8D6E8]/80 bg-[radial-gradient(circle_at_50%_42%,#FFFFFF_0%,#F1F6FC_58%,#E7EEF7_100%)] p-2 shadow-[0_10px_26px_rgba(38,66,105,0.09)] sm:p-3" onMouseEnter={() => setArchIsPaused(true)} onMouseLeave={() => setArchIsPaused(false)}>
            <div onClick={() => setIsArchImageModalOpen(true)} className="group relative block aspect-[16/9] w-full cursor-zoom-in overflow-hidden rounded-xl bg-[#EDF3FA] text-left">
              ...
              <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-3 sm:bottom-5 sm:left-5 sm:right-5">
                <button type="button" onClick={(event) => { event.stopPropagation(); setIsArchImageModalOpen(true); }} className="flex items-center gap-2 rounded-md border border-[#C5D4E8] bg-white/90 px-3 py-2 text-xs font-semibold text-slate-700 shadow-[0_4px_8px_rgba(31,41,55,0.08)] backdrop-blur-sm transition-colors hover:border-[#1459EB] hover:text-[#1459EB]">
                  <ZoomIn className="h-4 w-4 text-[#1459EB]" />
                  点击查看完整架构
                </button>
                <button
                  type="button"
                  ...
                  >
                    <div className="relative h-40 overflow-hidden bg-[#0C1D38]">
                      ...
                      <div className="absolute left-4 top-4 flex items-center gap-2">
                        ...
                        <span className="rounded-md bg-[#0C1D38]/72 px-2 py-1 text-[11px] font-medium text-blue-100/78 backdrop-blur-sm">{prod.code}</span>
                      </div>
                    ...
                    <div className="flex flex-1 flex-col p-5">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {prod.subTags.map((tag) => <span key={tag} className="rounded-md bg-slate-100 px-2 py-1 text-[11px] font-medium text-slate-600">{tag}</span>)}
                      </div>
                      <div className="mt-auto flex items-end justify-between border-t border-slate-100 pt-4">
                        <div>
                          <div className="font-number text-lg font-bold text-slate-950">{prod.metrics[0].value}</div>
                          <div className="mt-0.5 text-xs text-slate-500">{prod.metrics[0].label}</div>
                        </div>
          ...
          <HomeSectionHeading
            ...
            action={(
              <>
                ...
                <Link to="/ecology" className="inline-flex items-center gap-2 rounded-md border border-[#C5D2E2] bg-white px-5 py-3 text-sm font-semibold text-slate-700 transition-colors hover:border-[#1459EB] hover:text-[#1459EB]">了解开放生态<ChevronRight className="h-4 w-4" /></Link>
              </>
            )}
          />

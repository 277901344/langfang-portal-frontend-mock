...
const SCENARIO_FILTER_OPTIONS = [
  ...
  { label: '公积金专题数据方案', value: 'sc-001' },
  { label: '卫健医疗数据可信方案', value: 'sc-002' },
  { label: '法人信息跨域多维联控方案', value: 'sc-003' },
  { label: '不动产确权金融联办可信方案', value: 'sc-004' },
];
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="w-full lg:w-64 shrink-0 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
              ...
              {/* 行业分类 */}
              <div className="mb-5 border-t border-[#DDE3EC] pt-4">
                <p className="mb-2 text-[12px] font-medium text-[#8A94A6]">行业分类</p>
                <div className="flex flex-col gap-1">
                  {visibleIndustryCategoryOptions.map((opt) => (
                    <button
                      ...
                      className={cn(
                        "text-left px-3 py-1.5 rounded-[6px] text-[13px] transition-colors cursor-pointer flex items-center justify-between",
                        activeIndustryCategory === opt.value
                          ? "bg-[#E0E8F8] text-[#1459EB] font-semibold"
                          : "text-[#5B6472] hover:bg-[#F7F9FC] hover:text-[#1F2937] font-normal"
                      )}
                    >
                      <span>{opt.label}</span>
                      {opt.count !== undefined && (
                        <span className="text-[11px] text-[#8A94A6]">{opt.count}</span>
                      )}
                    </button>
                  ))}
                </div>
                {hiddenIndustryCategoryCount > 0 && (
                  <button
                    ...
                    className="mt-2 flex w-full items-center justify-center gap-1 rounded-[6px] border border-[#DDE3EC] py-1.5 text-[12px] font-medium text-[#1459EB] transition-colors hover:bg-[#E0E8F8]"
                  >
              ...
              <div className="border-t border-[#DDE3EC] pt-4">
                <p className="text-[12px] font-medium text-[#8A94A6] mb-2">交付方式</p>
                <div className="flex flex-col gap-1">
              ...
              <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
                {products.map((product, idx) => (
                  <motion.article
                    ...
                    className="group relative flex flex-col overflow-hidden rounded-[8px] border border-[#DDE3EC] bg-white p-5 shadow-[0_2px_8px_rgba(31,41,55,0.04)] transition-all duration-200 hover:-translate-y-0.5 hover:border-[#1459EB] hover:shadow-[0_4px_16px_rgba(31,41,55,0.08)]"
                  >
                    <div className="flex flex-1 flex-col">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="inline-flex items-center h-[22px] px-2 rounded-[6px] text-[12px] font-medium bg-[#E0E8F8] text-[#1459EB]">
                          {formatProductType(product.productTypeName || product.productType) || '未分类'}
                        </span>
                        {(product.industryCategoryName || product.industryCategory) && (
                          <span className="inline-flex min-w-0 items-center gap-1.5 text-[12px] font-medium text-[#5B6472]">
                        ...
                        {product.requiresTradingPlatform && (
                          <span className="inline-flex items-center gap-1 rounded-[4px] bg-[#E0E8F8] px-2 py-0.5 text-[11px] font-medium text-[#1459EB]">
                            需交易撮合
                          </span>
                        )}
                        {product.deliveryTypeName && (
                          <span className="ml-auto text-[12px] font-normal text-[#8A94A6]">
                            {product.deliveryTypeName}
                          </span>
                        )}
                      </div>

                      <h3 className="mt-3 line-clamp-1 text-[16px] font-semibold leading-[24px] text-[#1F2937]">
                        <Link to={`/products/${product.productId}`} className="transition-colors group-hover:text-[#1459EB]">
                          {product.productName}
                        </Link>
                      </h3>

                      <p className="mt-1.5 line-clamp-2 text-[13px] font-normal leading-[20px] text-[#5B6472]">
                        {product.description || '暂无产品描述'}
                      </p>

                      <div className="mt-3.5 grid grid-cols-2 divide-x divide-[#DDE3EC] border-y border-[#DDE3EC] py-2.5">
                        <div className="min-w-0 pr-4">
                          <div className="text-[11px] font-medium text-[#8A94A6]">应用场景</div>
                          <div className="mt-0.5 truncate text-[13px] font-medium text-[#1F2937]">{product.scenarioName || '-'}</div>
                        </div>
                        <div className="min-w-0 pl-4">
                          <div className="text-[11px] font-medium text-[#8A94A6]">交付方式</div>
                          <div className="mt-0.5 truncate text-[13px] font-medium text-[#1F2937]">
                    ...
                    <div className="mt-3 flex items-end justify-between gap-4">
                      <div className="min-w-0 space-y-1 text-[12px] font-normal text-[#8A94A6]">
                        <div className="flex items-center gap-1.5">
                          <Database className="h-3.5 w-3.5 shrink-0 text-[#1459EB]" />
                          <span className="truncate">{product.organizationName || product.connectorName || '-'}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                          <Clock className="h-3.5 w-3.5 shrink-0 text-[#1459EB]" />
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <button
                          ...
                          className="inline-flex items-center justify-center h-[32px] px-3.5 rounded-[6px] bg-[#1459EB] hover:bg-[#0E43B5] text-white text-[13px] font-medium transition-all cursor-pointer shadow-xs"
                        >
                          申请使用
                        </button>
                        <Link
                          to={`/products/${product.productId}`}
                          className="inline-flex items-center gap-0.5 text-[13px] font-medium text-[#5B6472] transition-colors hover:text-[#1459EB] px-2 py-1.5"
                        >
                          查看详情

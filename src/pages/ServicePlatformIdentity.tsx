...
export function ServicePlatformIdentity() {
  ...
  const [localStatus, setLocalStatus] = useState<number>(() => user?.authStatus ?? 2);
      ...
      <div className="w-full max-w-5xl space-y-4">
        ...
        <div className="flex items-center justify-between pb-1">
          ...
          <button
            type="button"
            onClick={() => navigate('/')}
            className="px-3 py-1.5 text-xs text-slate-600 hover:text-slate-900 bg-white border border-slate-200/90 hover:bg-slate-50 rounded-lg font-medium transition-colors cursor-pointer shadow-2xs hover:border-slate-300"
            title="退出认证"
          >
            退出认证
          </button>

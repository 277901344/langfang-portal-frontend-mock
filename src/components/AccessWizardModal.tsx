...
export function AccessWizardModal() {
  ...
  const [isMatchingTrading, setIsMatchingTrading] = useState(false);
  const [tradingStepMsg, setTradingStepMsg] = useState('');
  ...
  const totalSteps = 2 + (hasAuthMgmt ? 1 : 0) + (hasTradingPlatform ? 1 : 0);
  const readyCount =
    (isAuth ? 1 : 0) +
    (isConnector ? 1 : 0) +
    (hasAuthMgmt ? (isAuthorized ? 1 : 0) : 0) +
    (hasTradingPlatform ? (isTradingMatched ? 1 : 0) : 0);

  const canProceedToTrading = isAuth && isConnector && (!hasAuthMgmt || isAuthorized);
  ...
  const handleTradingMatchClick = async () => {
    setIsMatchingTrading(true);
    setTradingStepMsg('正在对接廊坊市数据交易平台撮合引擎...');

    setTimeout(() => {
      setTradingStepMsg('正在生成场内供需撮合意向单及交易清算结算协议...');
    }, 600);

    setTimeout(async () => {
      setTradingStepMsg('数据交易撮合签约成功！已登记挂牌流转号 LF-TRADE-2026-90412');
      await applyTradingMatch(targetProduct.productId);
      setIsMatchingTrading(false);
    }, 1200);
  };

  const handleViewTradingPlatform = () => {
    closeWizard();
    navigate('/platform/trading');
  };
  ...
  const showTradingStep = hasTradingPlatform && !isTradingMatched;
  const tradingPendingIndex = showTradingStep ? pendingStepCounter++ : 0;

  const pendingCount = (showAuthStep ? 1 : 0) + (showConnectorStep ? 1 : 0) + (showAuthMgmtStep ? 1 : 0) + (showTradingStep ? 1 : 0);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#1F2937]/50 backdrop-blur-xs animate-in fade-in-50">
      <div className="bg-white rounded-[12px] border border-[#DDE3EC] shadow-[0_4px_16px_rgba(31,41,55,0.12)] w-full max-w-xl overflow-hidden flex flex-col max-h-[92vh]">
          ...
          <div className="flex items-center gap-3 min-w-0 pr-2">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                ...
                {hasTradingPlatform && (
                  <span className="inline-flex items-center gap-1 text-[11px] font-medium text-[#1459EB] bg-[#E0E8F8] px-1.5 py-0.5 rounded">
        ...
        <div className="p-5 space-y-3.5 overflow-y-auto">
          ...
          {pendingCount === 0 && (
            <div className="p-6 rounded-[8px] bg-[#E8F6EE]/40 border border-[#159447]/30 text-center space-y-2">
              ...
              <p className="text-[13px] text-[#5B6472] max-w-md mx-auto">
                您的主体身份、数据连接器及相关授权/交易撮合条件均已满足，可直接点击下方按钮继续申请使用。
              </p>
            </div>
          )}
            ...
            >
              ...
              {isAuth ? (
                <button
                  ...
                  onClick={handleDeployConnectorClick}
          ...
          {/* 步骤: 数据交易平台交易撮合 (仅在需交易平台且未完成撮合时展示) */}
          {showTradingStep && (
            <div
              className={`p-3.5 rounded-[8px] border transition-all flex items-center justify-between gap-3 ${
                !canProceedToTrading
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className={`w-9 h-9 rounded-[6px] flex items-center justify-center shrink-0 font-medium text-sm ${
                    !canProceedToTrading ? 'bg-[#F7F9FC] text-[#8A94A6]' : 'bg-[#E0E8F8] text-[#1459EB]'
                  }`}
                >
                  {!canProceedToTrading ? (
                    <Lock className="w-4 h-4 text-[#8A94A6]" />
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-[14px] leading-[22px] font-medium text-[#1F2937]">
                      Step {tradingPendingIndex}: 数据交易平台交易撮合
                    </span>
                    <span className="inline-flex items-center h-[22px] px-2 rounded-[6px] bg-[#FEF5E7] text-[#D97706] text-[12px] font-medium">
                      待交易撮合
                    </span>
                  ...
                  <p className="text-[12px] leading-[18px] text-[#5B6472] truncate mt-0.5">
                    {!canProceedToTrading
                      ? '需先完成前序身份认证及连接器/授权步骤，方可发起交易撮合'
                      : '该产品属于场内市场化流通标的，需经数据交易平台完成交易撮合与结算签约'}
                  </p>
                </div>
              </div>
              ...
              {canProceedToTrading ? (
                <button
                  type="button"
                  disabled={isMatchingTrading}
                  onClick={handleTradingMatchClick}
                  className="shrink-0 bg-[#1459EB] hover:bg-[#0E43B5] text-white text-[13px] font-medium px-3 h-[32px] rounded-[6px] transition-colors cursor-pointer flex items-center gap-1.5 shadow-xs"
                >
                  {isMatchingTrading ? (
                    <>
          ...
          {isMatchingTrading && (
            <div className="p-3 bg-[#E0E8F8] border border-[#1459EB]/20 rounded-[6px] text-[13px] text-[#1459EB] flex items-center gap-2 animate-in fade-in">
              <Loader2 className="w-4 h-4 animate-spin shrink-0 text-[#1459EB]" />
              <span className="font-medium">{tradingStepMsg}</span>
            </div>
          )}

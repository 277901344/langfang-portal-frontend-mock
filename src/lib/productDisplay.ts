...
export function formatDiscountRate(discountPercent?: number) {
  if (!discountPercent || discountPercent >= 100) return '';
  // If discountPercent is 80 (meaning 80% price = 8折) or 50 (5折)
  const zhe = (discountPercent / 10).toFixed(1).replace(/\.0$/, '');
  return `${zhe}折`;
}
